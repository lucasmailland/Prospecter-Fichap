    ...
    async create(prospect) {
        return await this.tracingService.executeInSpan('prospect_repository.create', async () => {
            ...
            try {
                ...
                this.logger.log(`Prospecto creado exitosamente: ${savedProspect.id}`);
        ...
        return await this.tracingService.executeInSpan('prospect_repository.find_by_id', async () => {
            ...
            try {
                ...
                if (cached && typeof cached === 'string') {
                    this.logger.debug(`Prospecto obtenido del cache: ${id}`);
                ...
                this.logger.debug(`Prospecto obtenido de la base de datos: ${id}, encontrado: ${!!prospect}`);
                ...
                this.tracingService.markError(error);
                this.logger.error(`Error al obtener prospecto por ID ${id}: ${error.message}`);
                throw error;
            }
            finally {
                span.end();
    ...
    async findAll(page = 1, limit = 10) {
        return await this.tracingService.executeInSpan('prospect_repository.find_all', async () => {
            ...
            try {
                ...
                const cached = await this.cacheService.get(cacheKey);
                if (cached && typeof cached === 'string') {
                    this.logger.debug(`Prospectos obtenidos del cache: página ${page}, límite ${limit}`);
                    span.setAttribute('cache.hit', true);
                    this.tracingService.markSuccess();
                    return JSON.parse(cached);
                }
                ...
                this.logger.debug(`Prospectos obtenidos de la base de datos: página ${page}, límite ${limit}, total ${total}, count ${data.length}`);
                ...
                this.tracingService.markError(error);
                this.logger.error(`Error al obtener prospectos: página ${page}, límite ${limit}, error: ${error.message}`);
                throw error;
            }
            finally {
                span.end();
        ...
        return await this.tracingService.executeInSpan('prospect_repository.update', async () => {
            ...
            try {
                ...
                if (result.affected === 0) {
                    this.logger.warn(`Prospecto no encontrado para actualizar: ${id}`);
                ...
                this.logger.log(`Prospecto actualizado exitosamente: ${id}, campos actualizados: ${Object.keys(updateData).join(', ')}`);
                ...
                this.tracingService.markError(error);
                this.logger.error(`Error al actualizar prospecto ${id}: ${error.message}`);
                throw error;
            }
            finally {
                span.end();
    ...
    async delete(id) {
        return await this.tracingService.executeInSpan('prospect_repository.delete', async () => {
            ...
            try {
                ...
                if (result.affected === 0) {
                    this.logger.warn(`Prospecto no encontrado para eliminar: ${id}`);
                ...
                await this.cacheService.del('prospects:all');
                await this.cacheService.del('prospects:stats');
                this.logger.log(`Prospecto eliminado exitosamente: ${id}`);
                ...
                this.tracingService.markError(error);
                this.logger.error(`Error al eliminar prospecto ${id}: ${error.message}`);
                throw error;
            }
            finally {
                span.end();
