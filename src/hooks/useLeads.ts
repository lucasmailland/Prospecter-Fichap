...
export function useLeads(options: UseLeadsOptions = {}): UseLeadsReturn {
  ...
  const fetchLeads = useCallback(async (fetchOptions?: Partial<UseLeadsOptions>) => {
    if (!session?.user?.id) {
  ...
  }, [session?.user?.id, page, limit, filters, sort, includeUser, includeEnrichments]);
  
  // ========================================================================================
  // CRUD OPERATIONS
  // ========================================================================================
  
  const createLead = useCallback(async (data: CreateLeadForm): Promise<ApiResponse<Lead>> => {
    if (!session?.user?.id) {
  ...
  const updateLead = useCallback(async (id: string, data: UpdateLeadForm): Promise<ApiResponse<Lead>> => {
    try {
  ...
  const deleteLead = useCallback(async (id: string): Promise<ApiResponse<void>> => {
    try {
  ...
  const getLeadById = useCallback(async (id: string): Promise<ApiResponse<Lead>> => {
    try {
  ...
  const createBulkLeads = useCallback(async (leadsData: CreateLeadForm[]): Promise<ApiResponse<{ created: number; errors: string[] }>> => {
    if (!session?.user?.id) {
      return { success: false, error: 'Usuario no autenticado' };
    }
  ...
  const updateBulkLeads = useCallback(async (updates: Array<{ id: string; data: UpdateLeadForm }>): Promise<ApiResponse<{ updated: number; errors: string[] }>> => {
    try {
  ...
  const isEmailExists = useCallback(async (email: string): Promise<boolean> => {
    try {
  ...
  const getLeadStats = useCallback(async (): Promise<ApiResponse<any>> => {
    if (!session?.user?.id) {
      return { success: false, error: 'Usuario no autenticado' };
    }
